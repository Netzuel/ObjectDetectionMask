...
def main():
    ...
    interpreter = tf.lite.Interpreter(model_path = sys.argv[2])
    ...
    for i in range(len(v_boxes)):
        ...
        if v_labels[i] == "with_mask":
            ...
            image = cv2.putText(image, "mask"+" "+str(np.round(v_scores[i], 2)), (x1, y1-2), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
        elif v_labels[i] == "without_mask":
            image = cv2.rectangle(image, (x1,y1), (x2,y2), (0,0,255), 1)
            image = cv2.putText(image, "no_mask"+" "+str(np.round(v_scores[i], 2)), (x1, y1-2), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
        else:
            image = cv2.rectangle(image, (x1,y1), (x2,y2), (255,165,0), 1)
            image = cv2.putText(image, v_labels[i]+" "+str(np.round(v_scores[i], 2)), (x1, y1-2), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 165, 0), 1)
    cv2.imwrite("output.png", image)
    print("Exporting image as 'output.png'...")
